...
function Inventory({ data, busy, onCreate, onAdjust }: { data: CartData; busy: boolean; onCreate: (values: Record<string, unknown>) => Promise<boolean>; onAdjust: (values: Record<string, unknown>) => Promise<boolean> }) {
  ...
  return <div className="space-y-5"><section className={`${styles.panel} overflow-hidden`}><div className={`${styles.sectionBanner} flex flex-col gap-4 px-5 py-7 sm:flex-row sm:items-center sm:justify-between sm:px-7`}><div><span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-amber-300"><Box className="h-3.5 w-3.5" /> Control del carrito</span><h2 className="mt-3 text-3xl font-black tracking-tight">Inventario de premios</h2><p className="mt-1 text-sm text-blue-100">{lowStock.length ? `${lowStock.length} productos necesitan reposición` : "Todo listo para la próxima vuelta del carrito"}</p></div><button onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-300 px-4 py-3 text-sm font-black text-blue-950 shadow-lg"><PackagePlus className="h-4 w-4" />Agregar premio</button></div><div className="p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-5">{TIER_CONFIG.map((tier) => { const rewards = data.rewards.filter((reward) => reward.ticket_cost === tier.cost); return <section key={tier.cost} className={`overflow-hidden rounded-2xl border-2 ${tier.cost === 1 ? "border-emerald-200 bg-emerald-50/50" : tier.cost === 4 ? "border-sky-200 bg-sky-50/50" : "border-violet-200 bg-violet-50/50"}`}><div className="flex flex-col gap-3 border-b border-white bg-white/75 px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className={`grid h-11 w-11 place-items-center rounded-xl border bg-white text-lg font-black ${tierTone(tier.cost)}`}>{tier.cost}</span><div><h3 className="font-black text-blue-950">{tier.title}</h3><p className="text-xs text-slate-500">{tier.description}</p></div></div><button onClick={() => { setForm({ ...form, ticketCost: tier.cost }); setShowCreate(true); }} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-blue-950 px-3 py-2 text-xs font-black text-white"><Plus className="h-3.5 w-3.5" />Añadir premio de {tier.label}</button></div><div className="grid gap-3 p-3 sm:grid-cols-2 lg:grid-cols-3">{rewards.map((reward) => { const low = reward.stock <= reward.minimum_stock; return <article key={reward.id} className={`rounded-xl border bg-white p-3 shadow-sm ${low ? "border-rose-200" : "border-slate-100"}`}><div className="flex items-start justify-between gap-2"><h4 className="text-sm font-black text-blue-950">{reward.name}</h4><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${low ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}>{low ? "Comprar" : "Disponible"}</span></div>{reward.description ? <p className="mt-1 text-[11px] text-slate-500">{reward.description}</p> : null}<div className="mt-3 flex items-end justify-between"><div><span className="text-2xl font-black text-slate-950">{reward.stock}</span><span className="ml-1 text-[11px] font-bold text-slate-500">unidades</span><p className="text-[10px] text-slate-400">Alerta en {reward.minimum_stock}</p></div><button onClick={() => { setAdjusting(reward); setAdjustment({ delta: 1, kind: "purchase", note: "" }); }} className="grid h-9 w-9 place-items-center rounded-lg bg-slate-50 text-blue-950 ring-1 ring-slate-200" aria-label={`Actualizar stock de ${reward.name}`}><Plus className="h-4 w-4" /></button></div></article>; })}{!rewards.length ? <div className="col-span-full rounded-xl border border-dashed border-slate-200 bg-white/70 p-6 text-center"><Gift className="mx-auto h-6 w-6 text-slate-300" /><p className="mt-2 text-xs font-semibold text-slate-500">No hay premios registrados en este tier.</p></div> : null}</div></section>; })}</div>
      </div></section>
