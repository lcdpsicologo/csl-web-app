...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        existingRewards = parsed.slice(0, 200).flatMap((item) => {
          ...
          return id && name && TIERS.has(ticketCost) ? [{ id, name, ticketCost }] : [];
        });
      }
    } catch {
      existingRewards = [];
    ...
    const result = await callGemini({
      ...
      systemPrompt: `Eres una encargada experta en inventarios escolares. Analiza fotografías de premios para niñas y niños de Prekínder a 1° Básico. Identifica solamente objetos físicos claramente visibles que puedan guardarse como premios, agrupa objetos iguales y cuenta sus unidades visibles. Si una bolsa o caja cerrada no permite ver su contenido, cuéntala como una sola unidad. No inventes objetos ocultos ni cantidades. Decide un tier de 1, 4 u 8 tickets según el valor relativo del premio. Compara cada grupo con el catálogo existente y asigna existingRewardId únicamente cuando sea el mismo tipo de premio; ante la duda, déjalo vacío. Responde exclusivamente JSON con esta forma: {"items":[{"name":"nombre breve en español","description":"descripción breve","quantity":1,"ticketCost":1,"minimumStock":3,"existingRewardId":"o vacío","confidence":0.9}],"notes":"observación breve sobre el conteo"}.`,
      userPrompt: `Catálogo actual:\n${JSON.stringify(existingRewards)}\n\nAnaliza la foto. Devuelve como máximo 20 grupos. La cantidad debe representar unidades visibles, no una estimación del contenido oculto.`,
    ...
    const items = Array.isArray(payload.items) ? payload.items.slice(0, 20).flatMap((raw) => {
      ...
      if (!name) return [];
      const proposedTier = Number(raw.ticketCost);
      ...
      return [{
        ...
        ticketCost: TIERS.has(proposedTier) ? proposedTier as 1 | 4 | 8 : 1,
